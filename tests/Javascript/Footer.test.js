...
describe('Footer', () => {
	...
	test('it should not render back button on first step', () => {
		dontSee(backButtonText);
		see(cancelLinkText);
	});

	test('it shows back button on second step', async () => {
		stepForward();
	...
	test('it can back a step clicking a button', async () => {
		step.value = 2;
		await flushPromises();
	...
	test('it should not render cancel link when in last step', async () => {
		step.value = 4;
		await flushPromises();
